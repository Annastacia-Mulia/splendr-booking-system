...
import { StyleSheet, SafeAreaView, View, Text, TouchableOpacity, ScrollView, Dimensions, FlatList } from 'react-native';
...
const ClientHomeScreen = () => {
  ...
  useEffect(() => {
    if (selectedService) {
      axios.get(`${Config.API_URL}/api/businesses?service=${selectedService}`)
        .then(response => {
          console.log('Fetched businesses:', response.data); // Log the response data
          navigation.navigate('BusinessListScreen', { businesses: response.data });
        })
        .catch(error => {
          if (error.response) {
            console.error('Error fetching businesses (response):', error.response.data);
          } else if (error.request) {
            console.error('Error fetching businesses (request):', error.request);
          } else {
            console.error('Error fetching businesses (message):', error.message);
          }
        });
    }
  }, [selectedService]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>The Ultimate Beauty Booking Application</Text>
      </View>

      
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <View style={styles.buttonContainer}>
          <TouchableOpacity style={styles.boxButton} onPress={() => handlePressService('Hair')}>
            <Text style={styles.boxButtonText}>Hair</Text>
          </TouchableOpacity>

          <TouchableOpacity style={styles.boxButton} onPress={() => handlePressService('Barbering')}>
            <Text style={styles.boxButtonText}>Barbering</Text>
          </TouchableOpacity>

          <TouchableOpacity style={styles.boxButton} onPress={() => handlePressService('Nails')}>
        ...
        {selectedService && (
          <FlatList
            data={businesses}
            keyExtractor={item => (item.id ? item.id.toString() : Math.random().toString())} // Add check for item.id
